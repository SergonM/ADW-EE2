...
SELECT
    {{ dbt_utils.generate_surrogate_key(['r.rental_id']) }} AS rental_skey,
    dc.customer_skey,
    df.film_skey,
    ds.staff_skey,
    dst.store_skey,
    ...
    r.rental_duration
FROM rental_data r
JOIN {{ ref('dim_customer') }} dc ON r.customer_id = dc.customer_id
JOIN {{ ref('dim_film') }} df ON r.film_id = df.film_id
JOIN {{ ref('dim_staff') }} ds ON r.staff_id = ds.staff_id
JOIN {{ ref('dim_store') }} dst ON r.store_id = dst.store_id
